...
def pad_humidity(result: DatasetBuildIntermediate) -> DatasetBuildIntermediate:
    ...
    for group in groups.values():
        ...
        if rh_col is not None:
            ...
            var_attrs[new_col] = _build_attrs(source_attrs=var_attrs[rh_col], quantity='AH')
        else:
...
def pad_co2(result: DatasetBuildIntermediate) -> DatasetBuildIntermediate:
    ...
    for col, attrs in result.var_attrs.items():
        ...
        if ta_col is None:
            continue
        df[new_col] = get_calculation('CO2')(
            CO2c=df[col], Ta=df[ta_col], ps=df[ps_col]
        )
...
def _find_quantity_av(
    ...
    for var_name, attrs in var_attrs.items():
        if (attrs.get('quantity') == quantity and
                attrs.get('statistic_type') == StatisticType.AVG and
                var_name in df.columns):
            return var_name
    return None
...
def _find_closest_quantity_av(
    ...
    best_dist = float('inf')
    for var_name, attrs in var_attrs.items():
        if (attrs.get('quantity') != quantity or
                attrs.get('statistic_type') != StatisticType.AVG or
