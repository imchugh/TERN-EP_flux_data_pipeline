...
| `services/metadata/file_group_builder.py` | Builds `FileGroup` objects (master path, format, backup files) for all file groups in a site config. Use `build_file_groups(runtime_cfg)` when the target file group is not known in advance. **Known boundary leak (deferred, not yet fixed):** resolves each site's raw-data directory via `infrastructure.paths.get_local_stream_path(resource="raw_data", stream="flux_slow", ...)`, which is driven by TERN's `configs/paths.yml` naming convention — a site whose data lives outside that scheme cannot be processed through this function today, even though it's billed as generic core. |
...
Within the four architectural layers above, modules further sort into three groups relevant to a possible future extraction of a generic, network-agnostic EC processing core (see the split-decision history in project memory). This is documentation of an existing informal boundary, not a new physical split. Decision: keep this as an in-repo discipline for now; revisit real extraction only once the boundary has proven stable in practice. One misplaced-by-directory file (`data_monitor.py`) has been physically relocated to its correct tier as a low-risk step toward that eventual split — see the note under tier 3.

**1. Generic core** (portable to any EC flux-processing setup): `domain/`, `infrastructure/`, `services/data/` (including `data_monitor.py`'s analysis functions — `analyse_missing_data`, `analyse_variable_quality`, `analyse_threshold_quality` — generic EC data-quality/QC logic, same tier as `transform_service.py`), the core of `orchestration/` (`build_dataframe`, `build_dataset_from_context`, `derived_quantities.py`), and the schema/mechanism files in `services/metadata/`: `canonical_quantity_registry.py`, `site_config_schema.py`, `runtime_config_builder.py`, `file_group_builder.py`, `variable_name_parser.py`, `services/config_loader.py`. Contract objects (`SiteRuntimeConfig`, `SiteContext`, `VariableSpec`, `FileGroup`, `CanonicalQuantityMetadata`) are the seam: any adapter that produces these shapes can drive the core. Note: `data_monitor.py` still imports `services.metadata.site_registry` (TERN-adapter tier, see below) for site metadata — genuine tier purity would need that injected instead; deferred, not yet done. Note: `file_group_builder.py` still calls `infrastructure.paths` directly using TERN's `raw_data`/`flux_slow` resource-stream convention — deferred boundary leak, see its Key Modules row above.

**2. TERN-EP adapter** (produces the contract objects from TERN's sourcing): `services/metadata/site_registry.py`, `site_metadata_repository.py`, `instrument_registry.py`, `rdf_label_resolver.py`, `runtime_config_loader.py` (instrument-name validation *and* URI resolution against the TERN registry/RDF vocab happen here — see Architecture Decisions below; it delegates the actual generic assembly to `runtime_config_builder.py`, tier 1), the `*_from_site_name` convenience wrappers, and the TERN-specific config content in `configs/sites/*.yml`, `site_metadata.yml`, `nc_metadata.yml`, and the `remote:` section of `paths.yml`.
...
- Any TERN-specific resolution (RDF/SPARQL instrument lookups, vocab validation) must complete in the TERN-adapter layer (`services/metadata/runtime_config_loader.py` et al.) before contract objects reach the generic core — the core's functions (`build_dataframe`, `build_dataset_from_context`, `derived_quantities.py`) must never themselves call out to TERN's RDF/vocab store. Concretely, the core has two supported entry points into config assembly: `runtime_config_builder.build_runtime_config_from_file(file_path)` for a raw YAML with no TERN dependency at all, and `runtime_config_builder.build_runtime_config(validated_config, instrument_uris=None)` for an already-validated `SiteConfig` an adapter has optionally enriched — `runtime_config_loader.load_runtime_config(file_path)` is TERN's own adapter built on top of the latter.
