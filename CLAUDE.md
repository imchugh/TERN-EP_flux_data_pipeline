...
Within the four architectural layers above, modules further sort into three groups relevant to a possible future extraction of a generic, network-agnostic EC processing core (see the split-decision history in project memory). This is documentation of an existing informal boundary, not a new physical split — with one exception: `services/metadata/` itself *is* physically split into `services/metadata/core/` (tier 1) and `services/metadata/tern/` (tier 2) subpackages, a pure move-and-fix-imports rehearsal for the eventual real package boundary, no behaviour change. Decision: keep the boundary as in-repo discipline for now overall; revisit real extraction only once it has proven stable in practice. `orchestration/` is the harder remaining case for this same subpackage treatment if it's ever picked up — `dataframe_builder.py`/`dataset_builder.py` each mix tier-1 (`build_dataframe`, `build_dataframe_from_context`) and tier-2 (`build_dataframe_from_site_name`) *functions in the same file*, so splitting it means splitting file contents, not just moving files. One misplaced-by-directory file (`data_monitor.py`) has been physically relocated to its correct tier as a low-risk step toward that eventual split — see the note under tier 3. Also deferred to the split (to avoid adding a new pre-split file): extracting the shared incremental checkpoint/tail-append skeleton that `build_L1_zarr.update()` and `build_L2_zarr.update()` currently hand-roll separately — the `exists → seed / try → checkpoint-read → empty-check → append / except → full rebuild` envelope, including a verbatim-duplicated `_last_store_timestamp()`. Intended shape: one small higher-order helper (tier-1 generic core — no TERN in it) taking `produce_tail(checkpoint)` / `full_rebuild()` callbacks, with all level-specific logic (L2's MAD lookback window, post-QC tail slicing, tail-attrs refresh) staying in each caller. **Fold in at the same time — a config-change rebuild trigger.** Today nothing reacts to a site-config (or L2 QC-config) edit: `update()`'s `except → full rebuild` only catches *schema breakage* (a column added/removed breaking `to_zarr(mode="a")`), not *semantic drift* — an edit that changes how already-stored past records should be processed (unit conversion, calibration coefficient, instrument-period boundary, range limit, rename) appends cleanly and leaves the past stale until the next nightly `rebuild_L1_zarr` (up to ~24 h). Intended fix, as another branch of the shared envelope (`exists → seed / config-hash mismatch → full rebuild / try → checkpoint … / except → full rebuild`): stamp the store attrs at write time with a hash of the *assembled* config that drives processing (the `VariableSpec` set + instrument periods + conversions — not raw YAML text, so a comment/key-reorder edit doesn't trigger a rebuild), and have the helper force `full_rebuild()` when the current hash ≠ the stored one. Self-healing within one 30-min cycle, no file watcher or external trigger, same in-process `build()`-from-cron pattern the except-fallback already uses (so none of the concurrent-writer `write_zarr` concerns apply). Hash source is the per-caller injected bit: `SiteRuntimeConfig` for L1, the QC config for L2. Out of scope: `raw_data_loader.load_raw_data_since()`/`peek_last_timestamp()` (a layer-below primitive it depends on) and `tob_file_processor`/rclone (file-identity + hash dedup — a different incrementality model).
...
**Git mechanics when the extraction is actually done** (agreed plan, not yet executed):

1. **Prep stays on `main`.** The subpackage moves, import decoupling, and the deferred checkpoint helper land as normal small commits directly on `main` — each is independently safe and shrinks the eventual cutover.
2. **Decide history preservation upfront.** Default is *preserve* (keeps `git blame`/`git log` on the science code): use `git filter-repo` (or `git subtree split`) to carve each new repo with only its files' history, run *before* those files are deleted on the origin side. Only skip this if history genuinely doesn't matter, in which case a plain copy + fresh `git init` is simpler.
3. **New repos are a separate `git init`, not a branch** of this one. The filter-repo extraction is run once per new repo (shared-infra package, generic core, ops — however many tiers become separate artifacts).
4. **The origin-repo cutover is a short-lived branch + PR.** It does only the irreversible step: delete the extracted files, repoint imports at the new package(s), add the dependency. Rebase against `main` daily and merge within days, not weeks — file-move-heavy branches rot fast against an active `main` (live cron/operational traffic). One merge removes all extracted tiers together so `main` is never left half-split.
5. **Tag `main` `pre-split`** immediately before the cutover merge as a clean reference point.

**Path handling across the boundary** (agreed direction, partially done): the generic core takes filesystem locations as *data*, never by importing `infrastructure.paths`. `paths.py` + `paths.yml` are a templating system over TERN-EP's specific directory layout (site placeholders, stream structure, the `remote:` section) and stay on the TERN-orchestration side — not duplicated into the core, not given a core-side equivalent. A non-TERN caller supplies locations however it likes (its own config, CLI args, hardcoded `pathlib`); it does not inherit `paths.yml`.

- **Input locations**: on the contract objects — `SiteRuntimeConfig.input_data_path` (adapter-populated, `None` on the pure-core path; `file_group_builder` reads it and raises if unset, makes no `paths` calls). *Done.*
- **Output locations**: an explicit `output_dir` argument to the core build functions; the core owns only leaf/file naming below it (`{site}_L1.zarr`, `{site}_{year}_L1.nc`). *Partially done* — `build_L1_zarr` / `build_L1_nc` / `build_L2_zarr` accept `output_dir` but still fall back to `paths.get_local_stream_path(...)` defaults. Remaining work: delete those defaults, push the defaulting up into the TERN task wrappers in `tasks/build_tasks.py` (already tier-2). Endpoint is checkable: no `paths` import anywhere in the tier-1 core.
- **Convenience ergonomics**: `build_*_from_site_name()` "just works" from a bare site name only because `paths.py` expands everything — that one-call convenience is a TERN-adapter concern and already classified tier-2. The core keeps its explicit-args entry points (`build_dataframe`, `build_dataset_from_context`); no loss.
