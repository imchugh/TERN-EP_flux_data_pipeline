...
_BOYAGIN_CO2_COLS = [
    'Cc_LI840_0_5m', 'Cc_LI840_1m', 'Cc_LI840_3m', 'Cc_LI840_6m',
    'Cc_LI840_10m', 'Cc_LI840_16m', 'Cc_LI840_23m', 'Cc_LI840_30m',
    ]
...
def load_boyagin_profile() -> xr.Dataset:
    ...
    profile_dir = paths.get_local_stream_path(
        resource='raw_data', stream='profile', site='Boyagin',
        )
    met_dir = paths.get_local_stream_path(
        resource='raw_data', stream='flux_slow', site='Boyagin',
        )
    ...
    profile_df = (
        profile_df[profile_df.index.minute % 30 < 4]
        .resample('30min').mean()
        )
    ...
    p_series = _stack_to_series(
        pd.concat(
            [profile_df['P_atm_Avg']] * len(heights), axis=1, keys=heights,
            ) / 10,  # hPa -> kPa
        'P',
        )

    met_df = _load_toa5_with_backups(
        met_dir / _BOYAGIN_MET_FILE, usecols=['TIMESTAMP', 'Ta_HMP_Avg'],
        ).loc[_BOYAGIN_DATE_START:]
    ta_series = _stack_to_series(
        pd.concat(
            [met_df['Ta_HMP_Avg']] * len(heights), axis=1, keys=heights,
            ),
        'Tair',
        )
...
def _timestack_CUP_CO2(df: pd.DataFrame) -> pd.DataFrame:
    ...
    stacked = (
        df.set_index(pd.MultiIndex.from_frame(df[['Time', 'Height']]))['CO2_Avg']
        .unstack()
        )
...
def _interpolate_CUP_temperature(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.concat(
        ...
        axis=1, keys=_CUP_INTAKE_HEIGHTS,
        )
...
def load_cumberland_plain_profile() -> xr.Dataset:
    ...
    profile_dir = paths.get_local_stream_path(
        resource='raw_data', stream='profile', site='CumberlandPlain',
        )
    met_dir = paths.get_local_stream_path(
        resource='raw_data', stream='flux_slow', site='CumberlandPlain',
        )
    ...
    ep_master_df = raw_data_loader.load_raw_data(
        met_dir / _CUP_EP_MASTER_FILE, file_format='EddyPro',
        usecols=['date', 'time', 'air_pressure'],
        )
    p_series = _stack_to_series(
        pd.concat(
            [ep_master_df['air_pressure']] * len(_CUP_INTAKE_HEIGHTS), axis=1,
