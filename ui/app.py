...
def _get_file_vars(file_stem: str, fmt: str) -> list[str]:
    ...
    try:
        names = [
            v
            for v in get_variables_from_file(
                file_path=file_path, system_type=ftype.name
            )
...
def _long_name_for(var_name: str, var_data: dict) -> str:
    ...
    try:
        return (
            CANONICAL_REGISTRY.resolve_metadata(quantity, vtype, stat).long_name or ""
        )
    except ValueError:
...
def _make_table_edit_handler(var_name: str, refresh_fn) -> callable:
    ...
    def handle(row: dict) -> None:
        ...
        if new_file != old_file:
            attrs.update(
                {"file": new_file, "name": None, "instrument": None, "units": None}
            )
            refresh_fn()
            return
        ...
        if row.get("_instrument_is_compound"):
            attrs["instrument"] = {
                p["alias"]: p["name"] or None for p in row.get("_compound_parts", [])
            }
        else:
...
def render_variable(var_name: str, container) -> None:
    ...
    with container:
        ...
        with ui.row().classes("items-center gap-4 q-pb-sm"):
            ...
            ui.input(label="Horiz. location", value=hl).classes("w-32").props(
                "readonly"
            )
                    ...
                    with (
                        ...
                        for vname in sorted(cfg.get("variables", {}).keys()):
                            ...
                            with (
                                ...
                                .classes("bg-blue-100 text-blue-900" if active else "")
                            ):
...
def main():
    ...
    with ui.column().classes("w-full q-pa-md"):
        with ui.row().classes("items-center q-mb-md"):
            ...
            ui.label("Site").classes("text-caption text-grey-6").style(
                "margin-left: 60px; margin-right: 2px"
            )
            site_select = (
                ui.select(
                    options=sites,
                ...
                .props("borderless hide-selected dense")
                .style("min-width: 28px; max-width: 40px")
            )
        ...
        with ui.row().classes("q-mt-md gap-4"):
            ui.button(
                "Validate",
                on_click=lambda: do_validate(error_col),
            ).props("color=primary")
